...
function ProjectCard({ name, description, img, link, repo, stack = [] }) {
    ...
    return (
        ...
        >
            ...
            >
                <h3 className="text-slate-300 text-4xl md:text-6xl font-medium py-3">
                    {name}
                </h3>
                <p className="text-xl md:text-3xl py-3">{description}</p>
                <ul
                    className={`${spaceMono.className} grid grid-cols-3 gap-2 py-3`}
                >
                    {stack.map((tech, i) => (
                        <li key={`tech-${i + 1}`}>
